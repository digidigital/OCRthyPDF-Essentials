...
from pyxpdf.xpdf import PDFImageOutput
from pyzbar.pyzbar import decode, ZBarSymbol
from tempfile import TemporaryDirectory
from pikepdf import Pdf, PdfError, _cpphelpers 
from os import path
...
def analyzePage(PDF, pageNumber, separator):
    ...
    for pdfimage in PDFImageOutput(PDF).get(pageNumber):
                
        logger.debug('Extracting and analyzing an image on page %d' % (pageNumber+1))

        try:              
            barcodes = decode(pdfimage.image, symbols=[ZBarSymbol.QRCODE])
        except: 
            logger.debug('Decoding barcode failed. Skipping one image on page %d' % (pageNumber+1))
            continue
            
        for barcode in barcodes:
            barcodeText=str(barcode.data.decode("utf-8"))
            logger.info('QR-Code / Barcode containing text "%s" found on page %d. Use | as delimiter if you want to use a custom postfix' % (str(barcodeText), pageNumber+1))
            ...
            if len(barcodeComponents)==2 and barcodeComponents[0] == separator:
                ...
                logger.info('Ignored. Reason: "%s" on page %d does not start with separator "%s". Use | as delimiter if you want to use a custom postfix' % (str(barcodeText), pageNumber+1, separator))     
                continue
        
        #Skip remaining images if valid separator was found
...
def splitPDF(filename:str, outpath:str, separator='NEXT', stickerMode=False, dropName=False, noRepair=False):
    ...
    command = shlex.split("gs -o " + rewrittenPDF + gsQuiet + " -dNumRenderingThreads=8 -dBufferSpace=1000000000 -dBandBufferSpace=500000000 -sDEVICE=pdfwrite  -dPDFSETTINGS=/default '" + filename + "'")
    ...
    sourcePDF = Document(rewrittenPDF)
    ...
    separatePages=[]
    pdf = Document(rewrittenPDF)

    max_workers=cpu_count()-1

    logger.info('Analyzing pages with %d workers' % (max_workers))
    with concurrent.futures.ThreadPoolExecutor(max_workers) as executor:
        future_page_analyzer = {executor.submit(analyzePage, pdf, pageNumber, separator): pageNumber for pageNumber in range(len(pdf))}
    ...
    logger.info('Analysis completed: %d separators found on %d pages. This took about %d seconds'%(len(separatorPages),len(pdf), int(time.time() - startTime)))
